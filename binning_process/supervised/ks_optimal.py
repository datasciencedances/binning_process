...
class KSOptimalBinner(BaseBinner):
    """
    ...
    │                                                                  │
    │  KS đo khoảng cách lớn nhất giữa đường tích lũy (CDF) của bad    │
    │  và CDF của good. Cut-point tối ưu KS = điểm phân biệt           │
    │  bad/good tốt nhất tại một ngưỡng.                               │
    │                                                                  │
    │  Khác với IV (tối ưu "toàn bộ phân phối") — KS tối ưu "điểm      │
    │  phân biệt tốt nhất tại 1 ngưỡng". Phù hợp khi mục tiêu là       │
    │  tìm điểm cắt quyết định (reject/approve).                       │
    │                                                                  │
    │  Thuật toán: Chia đệ quy — mỗi bước tìm cut có KS lớn nhất       │
    │  trong đoạn, chia đôi, lặp lại trong từng đoạn con.              │
    └──────────────────────────────────────────────────────────────────┘
    """
    ...
    def _ks_at_cut(x: np.ndarray, y: np.ndarray, cut: float) -> float:
        """Tính KS statistic nếu cắt tại 'cut'."""
        left  = y[x <= cut]
        right = y[x >  cut]
    ...
    def _best_ks_cut(self, x: np.ndarray, y: np.ndarray,
        ...
        """Tìm cut-point có KS lớn nhất trong tập candidates."""
    ...
    def _recursive_ks(self, x: np.ndarray, y: np.ndarray,
        ...
        """Chia đệ quy, mỗi lần chọn cut có KS lớn nhất."""
        ...
        # Candidates: quantile của đoạn hiện tại
        pcts       = np.linspace(10, 90, 17)   # 17 điểm tránh biên
